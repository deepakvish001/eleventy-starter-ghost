...
Define the dependency policy standard for Ghostleaf so Ghost content is transformed into predictable, accessible, and deployable Eleventy output.
...
- The dependency policy requirement is observable at build time or in generated output.
...
Keep implementation work focused on one dependency policy outcome and record unrelated template or content-model changes separately.