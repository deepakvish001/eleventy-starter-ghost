...
Define the empty states standard for Ghostleaf so Ghost content is transformed into predictable, accessible, and deployable Eleventy output.
...
- The empty states requirement is observable at build time or in generated output.
...
Keep implementation work focused on one empty states outcome and record unrelated template or content-model changes separately.